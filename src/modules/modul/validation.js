...
const moduleCategoryListBody = body("module_category")
  .optional({ nullable: true, checkFalsy: true })
  .isArray()
  .withMessage("module_category harus berupa array")
  .bail()
  .custom((categories) =>
    categories.every(
      (category) =>
        typeof category === "string" && category.trim().length <= 20,
    ),
  )
  .withMessage("Setiap module_category harus berupa teks maksimal 20 karakter")
...
const chaptersBody = body("chapters")
  ...
  .custom((chapters) => {
    chapters.forEach((chapter, idx) => {
      ...
      if (
        !chapter.id &&
        (typeof chapter.title !== "string" || chapter.title.trim().length < 3)
      ) {
        throw new Error(
          `chapters[${idx}].title wajib diisi (minimal 3 karakter) untuk chapter baru`,
        );
      }
      if (
      ...
      if (
        chapter.link_materials !== undefined &&
        !Array.isArray(chapter.link_materials)
      ) {
        throw new Error(
          `chapters[${idx}].link_materials harus berupa array link`,
        );
      }
    });
    return true;
...
const listValidation = [
  ...
  moduleCategoryListBody,
  body("created_by")
    .customSanitizer((value) => {
      if (
        typeof value === "string" &&
        ["", "null", "nan"].includes(value.trim().toLowerCase())
      )
        return null;
      return value;
    })
