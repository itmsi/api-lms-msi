...
const roles = [
  ...
  { slug: "admin", name: "Admin", description: "Pengelola sistem LMS" },
  {
    slug: "instruktur",
    name: "Instruktur",
    description: "Pengajar pada sistem LMS",
  },
  {
    slug: "participant",
    name: "Participant",
...
const resources = [
  { key: "roles", label: "Role", endpoint: `${API_PREFIX}/roles` },
  {
    key: "permissions",
    label: "Hak Akses",
    endpoint: `${API_PREFIX}/permissions`,
  },
...
exports.seed = async function (knex) {
  ...
  const permissionId = Object.fromEntries(
    insertedPermissions.map((p) => [p.code, p.id]),
  );

  const rolePermissions = Object.entries(rolePermissionMap).flatMap(
    ([slug, codes]) =>
      codes.map((code) => ({
        role_id: roleId[slug],
